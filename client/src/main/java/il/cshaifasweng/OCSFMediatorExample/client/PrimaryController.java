...
public class PrimaryController {
	...
	private TextField workerid; // Value injected by FXMLLoader
	@FXML // fx:id="EditItemPrice"
	private TextField workername; // Value injected by FXMLLoader
	@FXML // fx:id="EditItemType"
	private TextField workeremail; // Value injected by FXMLLoader
	@FXML // fx:id="EditItemExtra"
	private TextField workerpassword; // Value injected by FXMLLoader

	@FXML // fx:id="UpdateItem"
	private Button workerapply; // Value injected by FXMLLoader
	@FXML // fx:id="UpdateItem"
	private Button workerapplyadd; // Value injected by FXMLLoader        @FXML // fx:id="UpdateItem"
	@FXML
	private Button workerapplyremove; // Value injected by FXMLLoader
	...
	@FXML
	void chooseAdminEditworker(ActionEvent event) {
		String chosen = worker_edit.getSelectionModel().getSelectedItem();
		if(chosen == "Add worker"){
			workerpassword.setVisible(true);
			workerid.setVisible(true);
			workername.setVisible(true);
			workeremail.setVisible(true);
			workerapplyadd.setVisible(true);
			workerapplyremove.setVisible(false);
			workerapply.setVisible(false);
		}else{
			if(chosen == "Remove worker"){
				workerid.setVisible(true);
				workerapplyremove.setVisible(true);
				workerapplyadd.setVisible(false);
				workerapply.setVisible(false);
				workername.setVisible(false);
				workeremail.setVisible(false);
				workerpassword.setVisible(false);
			}else{
				if(chosen == "Edit worker"){
					workerpassword.setVisible(true);
					workerid.setVisible(true);
					workername.setVisible(true);
					workeremail.setVisible(true);
					workerapply.setVisible(true);
					workerapplyadd.setVisible(false);
					workerapplyremove.setVisible(false);
				}
			}
	...
	@FXML
	void Workerapply(ActionEvent event) {
		//TODO: edit worker in DB


		String newName = workername.getText();
		String newEmail = workeremail.getText();
		String newPassword = workerpassword.getText();
		String workeridd = workerid.getText();
		int idd = Integer.valueOf(workeridd);
		// Create a new product with the current variables
		//Worker new_worker = new Worker("name","aaa@gmail","asdf",1);
		Worker new_worker = new Worker();
		new_worker.setFullName(newName);
		new_worker.setEmail(newEmail);
		new_worker.setPassword(newPassword);
		new_worker.setLoggedIn(false);
		new_worker.setPersonID(idd);
		UpdateMessage updateMessage2 = new UpdateMessage("worker", "edit");
		updateMessage2.setWorker(new_worker);
		updateMessage2.setId(idd);
		System.out.println("before try - edit worker");
		try {
			System.out.println("before sending updateMessage2222222222222 to server ");
			SimpleClient.getClient().sendToServer(updateMessage2); // sends the updated product to the server class
			System.out.println("afater sending updateMessage222222222222222 to server ");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		//TODO: add worker in DB
	...
	@FXML
	void Workeradd(ActionEvent event) {
		...
		new_worker.setLoggedIn(false);
		workernum2++;
		new_worker.setPersonID(workernum2);
		UpdateMessage updateMessage2 = new UpdateMessage("worker", "add");
		updateMessage2.setWorker(new_worker);
		updateMessage2.setId(workernum2);
		System.out.println("before try - edit worker");
		System.out.println(newName);
		System.out.println(newEmail);
		System.out.println(newPassword);
		System.out.println(workernum2);
		try {
			System.out.println("before sending updateMessage2222222222222 to server ");
			SimpleClient.getClient().sendToServer(updateMessage2); // sends the updated product to the server class
			System.out.println("afater sending updateMessage222222222222222 to server ");
	...
	@FXML
	void Workerremove(ActionEvent event) {
		String workeridd = workerid.getText();
		int idd = Integer.valueOf(workeridd);
		//TODO: remove worker in DB
		workerid.setVisible(false);
		workeremail.setVisible(false);
		workerpassword.setVisible(false);
		workername.setVisible(false);
	...
	{
		if(cartanchor.isVisible())
		{
			cartanchor.setVisible(false);

		}
		else
		{
			cartanchor.setVisible(true);
		}
